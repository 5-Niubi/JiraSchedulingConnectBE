...
{
    internal class ORExecution
    {
        public static void Schedule(OutputToORDTO data)
        {
            /// Pre-processing
            ...
            var solver = new CpSolver
            {
                ...
                + $"subsolvers:\"{CPPARAMS.SUB_SOLVER}\";"
            ...
            var pts = model.NewIntVar(0, int.MaxValue, "pts");
            ...
            foreach (var i in allTasks)
            {
                ...
                foreach (var j in pool)
                {
                    ...
                    var tmpExp = model.NewIntVar(0, 5 * data.NumOfSkills, $"tmpExp[{j}][{i}]");
            ...
            if (status == CpSolverStatus.Optimal || status == CpSolverStatus.Feasible)
            {
                ...
                {
                    var pool = taskWorkerPool.ElementAt(t);
                    Console.WriteLine($"Task {t} start at {solver.Value(ts[t])} end at {solver.Value(tf[t])}");
                    foreach (var w in pool)
                    {
                        if (solver.Value(A[(t, w)]) != 0)
                        {
                            Console.WriteLine($"Task {t} is assigned to {w}");
                        }
                    }
                }
                ...
                Console.WriteLine(JsonSerializer.Serialize(se, new JsonSerializerOptions { WriteIndented = true }));
            }
        }
    }
}
